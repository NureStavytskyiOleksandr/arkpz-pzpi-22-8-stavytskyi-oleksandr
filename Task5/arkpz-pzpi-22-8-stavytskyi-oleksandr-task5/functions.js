...
async function updateParkingSpotStatus(sensor_id, spot_status) {
    try {
        // Переконка, що spot_status є boolean
        if (typeof spot_status !== 'boolean') {
            throw new Error('Невірний формат spot_status');
        }
        ...
        const query = `
            ...
            RETURNING *;
        `;
        const values = [spot_status, sensor_id];

        const result = await client.query(query, values);
        return result.rows[0];  // Повертаємо оновлений запис
    } catch (err) {
...
const createParkingSpot = async (spotNumber, sensor_id, spotStatus, parkingGroupId) => {
    const query = `
        INSERT INTO parking_spots (spot_number, sensor_id, spot_status, parking_group_id)
        VALUES ($1, $2, $3, $4)
        RETURNING *;
    `;
    const values = [spotNumber, sensor_id, spotStatus, parkingGroupId];
...
const updateParkingSpot = async (parkingSpotId, spotNumber, sensor_id, spotStatus, parkingGroupId) => {
    const query = `
        UPDATE parking_spots
        SET spot_number = $1, sensor_id = $2 ,spot_status = $3, parking_group_id = $4
        WHERE parking_spot_id = $5
        RETURNING *;
    `;
    const values = [spotNumber, sensor_id, spotStatus, parkingGroupId, parkingSpotId];
