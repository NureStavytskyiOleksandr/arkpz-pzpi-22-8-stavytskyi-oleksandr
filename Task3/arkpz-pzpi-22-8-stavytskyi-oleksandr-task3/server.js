...
app.post('/parking-spots', async (req, res) => {
    const { spot_number, spot_status, parking_group_id } = req.body;

    try {
        const newSpot = await createParkingSpot(spot_number, spot_status, parking_group_id);
...
app.put('/parking-spots/:id', async (req, res) => {
    ...
    try {
        const updatedSpot = await updateParkingSpot(id, spot_number, spot_status, parking_group_id);
